...
function moveApp(app, loc) {
    // Force 3 columns at the start
    config.cols = 3;
    config.rows = 1;
    _log(`moveApp with forced cols:${config.cols} rows:${config.rows}`);
...
function enable() {
    ...
    keyManagerTimer = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 3000, () => {
        ...
        keyManager.add('<Super>left', () => {
            ...
            if (!app.wintile) initApp(app);
            _log("Left key pressed - moving to col 0");
            moveApp(app, { 'row': 0, 'col': 0, 'height': 1, 'width': 1 });
        });

        keyManager.add('<Super>down', () => {
            let app = global.display.focus_window;
            if (!app.wintile) initApp(app);
            _log("Down key pressed - moving to col 1");
            moveApp(app, { 'row': 0, 'col': 1, 'height': 1, 'width': 1 });
        });

        keyManager.add('<Super>right', () => {
            let app = global.display.focus_window;
            if (!app.wintile) initApp(app);
            _log("Right key pressed - moving to col 2");
            moveApp(app, { 'row': 0, 'col': 2, 'height': 1, 'width': 1 });
        });
        keyManager.add('<Super>up', () => {
            let app = global.display.focus_window;
            if (!app.wintile) initApp(app);
            moveApp(app, { 'row': 0, 'col': 0, 'height': config.rows, 'width': config.cols });
        });
        keyManager.add('<Super><Shift>left', () => {
            let app = global.display.focus_window;
            if (!app.wintile) initApp(app);
            moveApp(app, { 'row': 0, 'col': 0, 'height': config.rows, 'width': 0.75 });
        });

        keyManager.add('<Super><Shift>down', () => {
            let app = global.display.focus_window;
            if (!app.wintile) initApp(app);
            moveApp(app, { 'row': 0, 'col': 0.75, 'height': config.rows, 'width': 1.5 });
        });

        keyManager.add('<Super><Shift>right', () => {
            let app = global.display.focus_window;
            if (!app.wintile) initApp(app);
            moveApp(app, { 'row': 0, 'col': 2.25, 'height': config.rows, 'width': 0.75 });
        });
